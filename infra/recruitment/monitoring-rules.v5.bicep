...
// Return one row per matching telemetry record. Scheduled-query Count aggregation
// applies the threshold across the evaluation window.
...
var repeatedScanRetriesQuery = '''
union isfuzzy=true
  (traces | project TimeGenerated = timestamp, EventMessage = tostring(message)),
  (AppTraces | project TimeGenerated, EventMessage = tostring(Message))
| where EventMessage has "recruitment_scan_event_retry_requested"
| project TimeGenerated
'''
...
resource criticalProcessingFailures 'Microsoft.Insights/scheduledQueryRules@2023-12-01' = if (enableAlerts) {
  name: 'recruitment-critical-processing-failures-v5'
  location: location
  properties: {
    displayName: 'Recruitment critical processing failure'
    description: 'Alerts immediately on fail-closed configuration, retention purge/cleanup failure or finalization reconciliation failure.'
    enabled: true
    severity: 1
...
resource repeatedScanRetries 'Microsoft.Insights/scheduledQueryRules@2023-12-01' = if (enableAlerts) {
  name: 'recruitment-repeated-scan-retries-v5'
  location: location
  properties: {
    displayName: 'Recruitment repeated Defender scan retries'
    description: 'Warns when Defender scan processing requests redelivery at least three times within fifteen minutes.'
    enabled: true
    severity: 2
    evaluationFrequency: 'PT5M'
    windowSize: 'PT15M'
    ...
    criteria: {
      allOf: [
        {
          query: repeatedScanRetriesQuery
          timeAggregation: 'Count'
...
resource repeatedApiFailures 'Microsoft.Insights/scheduledQueryRules@2023-12-01' = if (enableAlerts) {
  name: 'recruitment-repeated-api-failures-v5'
  location: location
  properties: {
    displayName: 'Recruitment repeated API failures'
    description: 'Alerts when recruitment endpoints return five or more server errors in five minutes.'
    enabled: true
    severity: 2
    evaluationFrequency: 'PT5M'
    windowSize: 'PT5M'
...
resource readinessUnavailable 'Microsoft.Insights/scheduledQueryRules@2023-12-01' = if (enableAlerts) {
  name: 'recruitment-readiness-unavailable-v5'
  location: location
  properties: {
