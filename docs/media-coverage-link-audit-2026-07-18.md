...
- **Linked out to live originals (11):** AsianInvestor (Executive Exchange), PDI (Loan Note SC Lowy), HSBC (Finding opportunities), Nikkei Asia (Evergrande liquidation), P&I (Cambridge Associates), Investment Magazine (credit-crunch, 2021), SCMP (Opportunity of a lifetime), Bloomberg ($1.5trn minefield), PDI (Why lending is safer), Investment Magazine (special situations podcast, 2020), Week in China (No stranger to distress). Each row on both `media.html`/`media_cn.html` now points at the original article; the redundant local stub page + JSON were deleted.
