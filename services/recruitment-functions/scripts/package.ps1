...
try {
  ...
  foreach ($requiredRuntimeFile in @(
    ...
    'services/recruitment-functions/src/functions/bootstrap.js',
    'api/recruitment/core/flows.js'
  )) {
    if (-not (Test-Path -LiteralPath (Join-Path $stagingRoot $requiredRuntimeFile) -PathType Leaf)) {
      throw "Required runtime file is missing from deployment staging: $requiredRuntimeFile"
    }
  }
