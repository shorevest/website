(() => {
  ...
  <div class="sv-footer__inner">
    ...
    <div class="sv-footer__bottom">
      <nav class="sv-footer__links" aria-label="Footer legal links">
  ...
  <div class="sv-footer__inner">
    <div class="sv-footer__top">
      <div class="sv-footer__brandcol">
        <a class="sv-footer__logo" href="${siteHref('index_cn.html')}" aria-label="ShoreVest">
          <img src="${siteHref('assets/brand/sv-lockup-fc-light.png')}" alt="ShoreVest" width="200" height="48" />
        </a>
        <p class="sv-footer__offices"><span class="latin-text" lang="en">Guangzhou &middot; Shanghai &middot; Beijing &middot; Hong Kong</span><br /><a href="${siteHref('contact_cn.html')}">一般查询</a></p>
      </div>
      <nav class="sv-footer__nav" aria-label="页脚导航">
        <a href="${siteHref('firm_cn.html')}">公司</a>
        <a href="${siteHref('insights_cn.html')}">见解</a>
    ...
    <div class="sv-footer__legal">
      <p>本网站仅供一般信息参考之用，不构成出售要约、购买要约邀请，亦不构成任何证券或投资产品之推荐，或投资、法律或税务建议。</p>
      <p>过往业绩并不预示未来结果。私人信贷与私人债务投资涉及风险，包括可能损失本金。所列数据为约数，发布前有待确认。</p>
    </div>
    <div class="sv-footer__bottom">
      <nav class="sv-footer__links" aria-label="页脚法律链接">
        <a href="${siteHref('privacy-policy_cn.html')}">隐私政策</a>
        <a href="${siteHref('cookie-notice_cn.html')}"><span class="latin-text" lang="en">Cookie</span> 通知</a>
        <a href="${siteHref('terms-of-use_cn.html')}">使用条款</a>
        <a href="${siteHref('legal-notices-disclaimers_cn.html')}">法律声明及免责声明</a>
      </nav>
      <p class="sv-footer__copy">&copy; 2026 <span class="latin-text" lang="en">ShoreVest Partners</span>。版权所有。</p>
    </div>
  </div>`;
