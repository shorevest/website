#!/usr/bin/env python3
"""Apply approved Chinese copy to mirrored Chinese HTML pages without reformatting files."""
...
# Exact approved copy supplied for the site. Matching is performed against the
# mirrored English page, then applied at the same DOM location in the Chinese page.
STRUCTURAL_RULES_RAW = [
    ...
    # Standalone profile names use the form without 女士; inline biography fallbacks below add 女士 where requested.
    ("Louisa Ho", "何瑞青"),
    ("Esther Li", "李泳琪"),
    ("Charlotte Lan", "兰玉芳"),
...
# Inline fallbacks for proper nouns and abbreviations that can appear inside longer bios.
...
class SourceParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        element = Element(tag.lower(), {k: (v or "") for k, v in attrs}, self.stack[-1])
    ...
    def handle_data(self, data: str) -> None:
        ...
        self.stack[-1].children.append(TextNode(data, start, start + len(data), self.stack[-1]))

    def handle_entityref(self, name: str) -> None:
        raw = f"&{name};"
        start = self.absolute_position()
        self.stack[-1].children.append(TextNode(raw, start, start + len(raw), self.stack[-1]))
...
def walk_elements(root: Element) -> Iterable[Element]:
...
def walk_text_nodes(root: Element, skipped: bool = False) -> Iterable[TextNode]:
    skipped = skipped or root.tag in SKIP_TEXT_TAGS
    for child in root.children:
        if isinstance(child, TextNode):
            if not skipped:
                yield child
        else:
            yield from walk_text_nodes(child, skipped)


def rendered_text(element: Element) -> str:
    return "".join(unescape(node.raw) for node in walk_text_nodes(element))


def find_body(root: Element) -> Element:
...
def find_by_id(root: Element, target_id: str) -> Element | None:
    for element in walk_elements(root):
...
def element_path(ancestor: Element, descendant: Element) -> list[int] | None:
    indexes: list[int] = []
    current = descendant
    while current is not ancestor:
        ...
        try:
            indexes.append(siblings.index(current))
        except ValueError:
            return None
        current = parent
...
def follow_path(ancestor: Element, indexes: list[int]) -> Element | None:
    current = ancestor
    for index in indexes:
        children = current.element_children
...
def locate_corresponding(source: Element, source_root: Element, target_root: Element) -> Element | None:
    anchor = source
    while anchor.parent is not None and not anchor.attrs.get("id") and anchor.tag != "body":
        anchor = anchor.parent
    if anchor.attrs.get("id"):
        target_anchor = find_by_id(target_root, anchor.attrs["id"])
        if target_anchor is not None:
            path = element_path(anchor, source)
            if path is not None:
                candidate = follow_path(target_anchor, path)
                if candidate is not None and candidate.tag == source.tag:
                    return candidate

    source_body = find_body(source_root)
    target_body = find_body(target_root)
    path = element_path(source_body, source)
    if path is not None:
        candidate = follow_path(target_body, path)
        if candidate is not None and candidate.tag == source.tag:
            return candidate
    return None


def preserve_outer_whitespace(raw: str, replacement: str) -> str:
    leading = re.match(r"^\s*", raw).group(0)
    trailing = re.search(r"\s*$", raw).group(0)
    return leading + replacement + trailing


def whitespace_only(raw: str) -> str:
    return "".join(char for char in raw if char.isspace())


def chinese_counterpart(path: Path) -> Path | None:
    if path.name.endswith("_cn.html"):
        return None
    if path.name == "index.html":
        candidate = path.with_name("index_cn.html")
    else:
        candidate = path.with_name(path.stem + "_cn.html")
    return candidate if candidate.exists() else None


def apply_structural_rules(english_source: str, chinese_source: str, rules: dict[str, tuple[str, str]]) -> tuple[str, set[str], int]:
    english_parser = SourceParser(english_source)
    english_parser.feed(english_source)
    chinese_parser = SourceParser(chinese_source)
    chinese_parser.feed(chinese_source)

    english_elements = list(walk_elements(english_parser.root))
    matches: dict[str, list[Element]] = {key: [] for key in rules}
    for element in english_elements:
        key = canonical(rendered_text(element))
        if key not in rules:
            continue
        # Keep the narrowest matching element to avoid replacing a wrapper that
        # contains a child with the same complete text.
        if any(canonical(rendered_text(child)) == key for child in element.element_children):
            continue
        matches[key].append(element)

    replacements: list[tuple[int, int, str]] = []
    matched: set[str] = set()
    used_targets: set[int] = set()

    for key, source_elements in matches.items():
        if not source_elements:
            continue
        _, approved = rules[key]
        for source_element in source_elements:
            target = locate_corresponding(source_element, english_parser.root, chinese_parser.root)
            if target is None or id(target) in used_targets:
                continue
            text_nodes = [node for node in walk_text_nodes(target) if canonical(unescape(node.raw))]
            if not text_nodes:
                continue
            replacements.append((text_nodes[0].start, text_nodes[0].end, preserve_outer_whitespace(text_nodes[0].raw, approved)))
            for node in text_nodes[1:]:
                replacements.append((node.start, node.end, whitespace_only(node.raw)))
            used_targets.add(id(target))
            matched.add(key)

    for start, end, replacement in sorted(replacements, key=lambda item: item[0], reverse=True):
        chinese_source = chinese_source[:start] + replacement + chinese_source[end:]
    return chinese_source, matched, len(replacements)


def apply_direct_text_replacements(source: str) -> tuple[str, int]:
    parser = SourceParser(source)
    parser.feed(source)
    replacements: list[tuple[int, int, str]] = []
    count = 0
    ordered = sorted(DIRECT_TEXT_REPLACEMENTS, key=lambda item: len(item[0]), reverse=True)

    for node in walk_text_nodes(parser.root):
        updated = node.raw
        for old, new in ordered:
            if old in updated:
                ...
                count += occurrences
        if updated != node.raw:
            replacements.append((node.start, node.end, updated))
...
def main() -> None:
    ...
    for english, chinese in STRUCTURAL_RULES_RAW:
        key = canonical(english)
        existing = structural_rules.get(key)
        if existing and existing[1] != chinese:
            raise RuntimeError(f"Conflicting approved translations for: {english}")
        structural_rules[key] = (english, chinese)
    ...
    for english_path in sorted(ROOT.rglob("*.html")):
        if ".git" in english_path.parts:
            continue
        ...
        original_chinese = chinese_path.read_text(encoding="utf-8")
        updated_chinese, matched, replacement_count = apply_structural_rules(
            english_source, original_chinese, structural_rules
        )
        updated_chinese, direct_count = apply_direct_text_replacements(updated_chinese)

        total_files += 1
        total_structural += replacement_count
        total_direct += direct_count
        globally_matched.update(matched)

        if updated_chinese != original_chinese:
            chinese_path.write_text(updated_chinese, encoding="utf-8")
            changed_files.append(str(chinese_path.relative_to(ROOT)))

    unmatched = [structural_rules[key][0] for key in structural_rules if key not in globally_matched]
    print(f"Scanned {total_files} mirrored page pairs")
    print(f"Changed {len(changed_files)} Chinese HTML files")
    print(f"Applied {total_structural} structural text-node edits and {total_direct} inline replacements")
    for path in changed_files:
        print(f"CHANGED {path}")
    if unmatched:
        print(f"UNMATCHED STRUCTURAL RULES ({len(unmatched)}):")
        for text in unmatched:
            print(f"- {text}")

    if not changed_files:
        raise SystemExit("No Chinese HTML files changed; refusing to create an empty commit")
