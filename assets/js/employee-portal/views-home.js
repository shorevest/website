...
(function (root) {
  ...
  SVOps.views.myWork=function(c,user){
    var p=persona(user), pg=page('My Work','My Work','One row means one action, one owner and one next step. Start with Do now.');
    pg.appendChild(el('div',{class:'ops-next-action'},[el('strong',{text:'Primary next action: '}),el('span',{text:'Open the first Do now item and either complete it, hold it with a reason, or assign the blocker.'})]));
    pg.appendChild(el('div',{class:'ops-filterbar'},['Do now','Waiting on others','Review suggestions','On hold','Overdue','Done'].map(function(f,i){return el('button',{class:'btn btn--quiet'+(i===0?' is-active':''),title:f==='Review suggestions'?'AI or vendor suggestions that need human acceptance before they become official work.':'Queue: '+f,text:f,onclick:function(){U.toast(f+' queue selected.');}})})));
    var rows=(p.home.needsYou||[]).map(function(x,i){return{type:primaryAction(x),task:x.title,owner:user.name,due:i?'2026-07-14':'Today',source:'Salesforce + Outlook + SharePoint',why:x.context&&x.context[0]||x.recommendation,status:i%2?'Needs review':'Ready',next:x.recommendation,open:'Open workflow'};}).concat((p.home.waiting||[]).map(function(x){return{type:'Wait',task:x.title,owner:user.name,due:'This week',source:'Owner queue',why:x.note,status:'Waiting',next:x.note,open:'Open item'};}));
    pg.appendChild(U.table([{key:'type',label:'Action',html:function(r){return U.actionChip(r.type)}},{key:'task',label:'Task',html:function(r){return esc(r.task)}},{key:'owner',label:'Owner',html:function(r){return esc(r.owner)}},{key:'due',label:'Due',html:function(r){return esc(r.due)}},{key:'source',label:'Source',html:function(r){return esc(r.source)}},{key:'why',label:'Why it matters',html:function(r){return esc(r.why)}},{key:'status',label:'Status',html:function(r){return U.statusHtml(r.status)}},{key:'next',label:'Next step',html:function(r){return esc(r.next)}},{key:'open',label:'Open item',html:function(r){return '<a href="#/home">'+esc(r.open)+'</a>';}}],rows));
