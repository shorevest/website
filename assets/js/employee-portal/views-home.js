...
(function (root) {
  ...
  SVOps.views.relationships=function(c,user){
    ...
    pg.appendChild(el('div',{class:'work-summary relationship-summary'},[
      summary('Needs review',2),summary('On hold',1),summary('Waiting',1),summary('Healthy',8)
    ]));
    pg.appendChild(el('div',{class:'ops-next-action relationship-start'},[
      ...
      el('button',{type:'button',class:'btn btn--primary',text:'Review first relationship',onclick:function(){openRelationship(firstReview(rel));}})
    ]));
    ...
    function summary(label,num){return el('div',{class:'work-summary__item'},[el('span',{text:label}),el('strong',{text:String(num)})]);}
    function firstReview(list){return list.filter(function(r){return r.status==='Needs review'||r.status==='Blocked';})[0]||list[0];}
    function mineName(){return user&&user.name||'';}
    function shown(){return rel.filter(function(r){if(filter==='All')return true;if(filter==='Mine only')return r.owner===mineName();if(filter==='Healthy')return r.group==='Healthy / no action needed';return r.status===filter||r.group===filter;});}
    function draw(){filterButtons.forEach(function(b){b.className='btn btn--quiet'+(b.textContent===filter?' is-active':'');});sectionsWrap.innerHTML='';['Needs review','On hold','Waiting','Healthy / no action needed'].forEach(function(group){var rows=shown().filter(function(r){return r.group===group;});if(rows.length){sectionsWrap.appendChild(section(group,rows));}});tableWrap.innerHTML='';tableWrap.appendChild(table(shown()));}
  ...
  function relationshipsFor(role){
    ...
    function r(o){return Object.assign({group:o.status==='On hold'?'On hold':o.status==='Waiting'?'Waiting':o.status==='Healthy'?'Healthy / no action needed':'Needs review',noticed:o.noticed||o.issue,blocker:o.blocker||'None recorded',review:o.review||'2026-07-17',actions:base.actions,sources:base.sources,evidence:o.evidence||['Salesforce stage: '+o.stage,'Owner judgement: '+(o.judgement||'needs confirmation'),'Recent activity: '+(o.activity||'not enough recent activity')],history:o.history||['Relationship exception created.','No official Salesforce writeback has occurred.']},o);}
    var rows={
