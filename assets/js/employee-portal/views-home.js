...
(function (root) {
  ...
  var maturity = ['Idea','Designed','Simulated','Connected','Validated','Production-ready'];
  ...
  SVOps.views.myWork=function(c,user){ var p=persona(user), pg=page('My Work','Assigned work for '+user.name,'Only human-accepted work items, exceptions and review decisions. AI suggestions stay Suggested until accepted.'); var grid=el('div',{class:'ops-grid ops-grid--2'}); (p.home.needsYou||[]).concat(p.home.waiting||[]).forEach(function(x,i){grid.appendChild(card(x.title||x.id,(x.recommendation||x.note)+' Source, evidence, owner and next action are shown before anything becomes official.', i%2?'Held':'Suggested'));}); pg.appendChild(grid); pg.appendChild(doctrine()); c.appendChild(pg); };
  SVOps.views.relationships=function(c){ var pg=page('Relationships','Relationship control layer','Not a replacement CRM: Salesforce stage and account state remain source fields, with objective evidence and owner actions visible.'); pg.appendChild(el('div',{class:'ops-panel'},[el('h2',{class:'ops-panel__title',text:'Account / LP relationship state'}), U.table(['Account / LP','Salesforce stage','Regional-owner priority','Objective engagement evidence','Challenge signal','Stale warning','Strategic barrier','Action plan','Action items','Waiting condition','Review date','Outcome'].map(function(k){return{key:k,label:k,html:function(r){return esc(r[k]);}}}),[{ 'Account / LP':'ATP','Salesforce stage':'Prospecting','Regional-owner priority':'John review','Objective engagement evidence':'2 opens, prior meeting note, source freshness shown','Challenge signal':'Six contacts found','Stale warning':'Next step stale 41 days','Strategic barrier':'Institution concentration','Action plan':'Include two, hold four','Action items':'Human owner review','Waiting condition':'John decision','Review date':'2026-07-17','Outcome':'Suggested' }]) ])); pg.appendChild(doctrine()); c.appendChild(pg); };
  SVOps.views.meetings=function(c){ var pg=page('Meetings','Meeting workspace','Concise cumulative briefs, meeting-specific preparation and post-meeting action extraction suggestions.'); pg.appendChild(el('div',{class:'ops-grid ops-grid--2'},[card('Calendar event','Address / meeting link validation, attendees and timing conflicts.','Simulated'),card('Relationship brief','Concise cumulative brief, prior concerns and objective.','Designed'),card('Post-meeting update','Suggested action extraction; human acceptance required before task creation.','Simulated')])); c.appendChild(pg); };
  SVOps.views.diligence=function(c){ var pg=page('Diligence & Requests','Diligence and requests','DDQs, data requests, answer bank, governed facts, materials, review/approval state and evidence chain.'); pg.appendChild(el('div',{class:'ops-grid ops-grid--3'},[card('DDQs','Section state, evidence chain and review queue.','Simulated'),card('Answer bank','Governed facts with source and freshness.','Designed'),card('Materials','Approved masters and controlled derivatives.','Connected')])); c.appendChild(pg); };
  SVOps.views.investorIntelligence=function(c){ var pg=page('Investor Intelligence','Investor Intelligence','Live weekly digest workstream plus market, intermediary and institution context.'); pg.appendChild(el('div',{class:'ops-grid ops-grid--3'},[card('Weekly investor digest','Live/Production workstream.','Production-ready'),card('Placement-agent assessment','Intermediary notes with evidence status.','Designed'),card('Regional coverage gaps','Coverage gaps and local decision-maker context.','Simulated'),card('Market intelligence','Source, owner and freshness visible.','Connected')])); c.appendChild(pg); };
  SVOps.views.reporting=function(c){ var pg=page('Reporting','Reporting','Weekly Outreach & Coverage Snapshot and live reporting exceptions.'); pg.appendChild(el('div',{class:'ops-grid ops-grid--3'},[card('Weekly Outreach & Coverage Snapshot','Live weekly digest/reporting item.','Production-ready'),card('Stale records','Missing next steps and data-quality exceptions.','Connected'),card('Activity summary','Activity and stage movement reporting; no automatic stage movement.','Simulated')])); c.appendChild(pg); };
